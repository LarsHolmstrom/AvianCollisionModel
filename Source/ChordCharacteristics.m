...
if isempty(section_1_r)
    c_1 = [];
else
    c_1 = chord_length_at_hub*(0.115*R/max_chord_length)*linspace(1,max_chord_length/chord_length_at_hub,length(section_1_r));
end
...
chord_angle = mod(chord_angle,360);
max_chord_angle = (0.640*atan(1./(8*(.1-0.015)))-0.073) * 360/(2*pi);
chord_angle(find(sample_points < bounds(2))) =max_chord_angle;
